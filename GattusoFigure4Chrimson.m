function res = GattusoFigure4Chrimson(data)
...
for n=1:length(data)
    ...
    if n==1, ylabel('ang speed (deg/s)'); end
...
for n=1:length(data)
    ...
    xticklabels({'OFF','ON'})
    box off; set(gca,'TickDir','out');
    hold on; plot([1 2],[nanmean(cbase) nanmean(clight)],'ro-','lineWidth',2,'markerSize',8);
...
for n=1:length(data)
    ...
    hold on; semilogy([0.5:1:30],nanmean(vlighthist),'r','lineWidth',2);
    
    hold on; semilogy([0.5:1:30],nanmean(vbasehist)+nanstd(vbasehist)/sqrt(length(dataset)),'k');
    hold on; semilogy([0.5:1:30],nanmean(vbasehist)-nanstd(vbasehist)/sqrt(length(dataset)),'k');

    hold on; semilogy([0.5:1:30],nanmean(vlighthist)+nanstd(vlighthist)/sqrt(length(dataset)),'r');
    hold on; semilogy([0.5:1:30],nanmean(vlighthist)-nanstd(vlighthist)/sqrt(length(dataset)),'r');
...
for n=1:length(data)
    ...
    hold on; semilogy([25:50:600],nanmean(alighthist),'r','lineWidth',2);
    
    hold on; semilogy([25:50:600],nanmean(abasehist)+nanstd(abasehist)/sqrt(length(dataset)),'k');
    hold on; semilogy([25:50:600],nanmean(abasehist)-nanstd(abasehist)/sqrt(length(dataset)),'k');

    hold on; semilogy([25:50:600],nanmean(alighthist)+nanstd(alighthist)/sqrt(length(dataset)),'r');
    hold on; semilogy([25:50:600],nanmean(alighthist)-nanstd(alighthist)/sqrt(length(dataset)),'r');
...
for n=1:length(data)
    ...
    xticklabels({'OFF','ON'})
    box off; set(gca,'TickDir','out');
    hold on; plot([1 2],[nanmean(curvOFF) nanmean(curvON)],'ro-','lineWidth',2,'markerSize',8);
...
for n=1:length(data)
    ...
    xticklabels({'OFF','ON'})
    box off; set(gca,'TickDir','out');
    hold on; plot([1 2],[nanmean(angvOFF) nanmean(angvON)],'ro-','lineWidth',2,'markerSize',8);

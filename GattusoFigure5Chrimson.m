function res = GattusoFigure5Chrimson(data)
...
if nargin <1
    ...
    load('/Users/kvanhass/Library/CloudStorage/OneDrive-BowdoinCollege/Documents/Nagel Lab/LAL light pulse mat files/Chrimson/LAL073 Chrimson.mat')
        data(2) = LAL073Chrimson;
    load('/Users/kvanhass/Library/CloudStorage/OneDrive-BowdoinCollege/Documents/Nagel Lab/LAL light pulse mat files/Chrimson/ss41947_chrimsoncorrect.mat')
        data(3) = ss41947_chrimsoncorrect;
    load('/Users/kvanhass/Library/CloudStorage/OneDrive-BowdoinCollege/Documents/Nagel Lab/LAL light pulse mat files/Chrimson/SS38176_Chrimson.mat')
        data(4) = SS38176_Chrimson;
    load('/Users/kvanhass/Library/CloudStorage/OneDrive-BowdoinCollege/Documents/Nagel Lab/LAL light pulse mat files/Chrimson/PDBgal4_Chrimson.mat')
        data(5) = PDBgal4_Chrimson;
...
for n=1:length(data)
    ...
    hold on; plot([1 2],[nanmean(vbase) nanmean(vlight)],'ro-','lineWidth',2,'markerSize',8);
...
for n=1:length(data)
    ...
    xticklabels({'OFF','ON'})
    box off; set(gca,'TickDir','out');
    hold on; plot([1 2],[nanmean(abase) nanmean(alight)],'ro-','lineWidth',2,'markerSize',8);
...
for n=1:length(data)
    ...
    hold on; semilogy([0.5:1:30],mean(vlighthist),'r','lineWidth',2);
    
    hold on; semilogy([0.5:1:30],mean(vbasehist)+std(vbasehist)/sqrt(length(dataset)),'k');
    hold on; semilogy([0.5:1:30],mean(vbasehist)-std(vbasehist)/sqrt(length(dataset)),'k');

    hold on; semilogy([0.5:1:30],mean(vlighthist)+std(vlighthist)/sqrt(length(dataset)),'r');
    hold on; semilogy([0.5:1:30],mean(vlighthist)-std(vlighthist)/sqrt(length(dataset)),'r');
...
for n=1:length(data)
    ...
    hold on; plot(t,nanmean(vON,2),'r','lineWidth',2);
    
    hold on; semilogy(t,nanmean(vOFF,2)+nanstd(vOFF,0,2)/sqrt(length(lightOFF)),'k');
    hold on; semilogy(t,nanmean(vOFF,2)-nanstd(vOFF,0,2)/sqrt(length(lightOFF)),'k');

    hold on; semilogy(t,nanmean(vON,2)+nanstd(vON,0,2)/sqrt(length(lightON)),'r');
    hold on; semilogy(t,nanmean(vON,2)-nanstd(vON,0,2)/sqrt(length(lightON)),'r');
    axis([0 70 0 11]);
    box off; set(gca,'TickDir','out');
